...
#################################tidy data#############################
library(readr)
genomes_data <- read_csv("../../input_raw/KO_count_grouped.csv") %>% data.frame()
colnames(genomes_data)[1]<-"features"
genomes_data$features=genomes_data$features %>% make.names()
genomes_data=column_to_rownames(genomes_data,"features")
...
pdf(glue::glue(project_name,"best.model_draw_feature_importance_plot.pdf"),width=12,height = 15)
